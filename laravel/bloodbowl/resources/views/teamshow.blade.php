...
<tr>
    ...
    <td><form id="player{{$player->id}}" method="POST" action="/player/{{$player->id}}"> @method('PATCH') @csrf <input class="form-control form-control-sm" type="text" name="name" placeholder="" value="{{$player->name}}"></form></td>
    ...
    {{-- <form method="POST" action="/player/{{$player->id}}">
        @method('PATCH')
        @csrf    --}}
        <button form="player{{$player->id}}" type="submit" class="btn btn-primary"><i class="fas fa-edit"></i></button>
    {{-- </form> --}}
    </td>
    <td>   
            <form method="POST" action="/player/{{$player->id}}">
